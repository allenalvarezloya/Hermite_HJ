...
clear, clc;
lambdavec = 0:0.1:4;
CFLvec = 0.01:0.01:0.1;
lambdaCFLmat = zeros(length(CFLvec),length(lambdavec))+100;
COUNTCFL = 1;
COUNTlambda = 1;
load('viscositySol.mat');
for CFL = CFLvec
...
%         saveas(gcf,'Example_04_1D','epsc')
        ...
        if conv1 > 0.8 && conv2 > 0.8 && convinf > 0.8 && Linferror(end) < 0.1
            lambdaCFLmat(COUNTCFL,COUNTlambda) = 0;
        elseif isnan(conv1) || isnan(conv2) || isnan(convinf)
            lambdaCFLmat(COUNTCFL,COUNTlambda) = -1;
        else
            lambdaCFLmat(COUNTCFL,COUNTlambda) = 1;
        end
